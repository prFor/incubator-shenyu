...
/**
 * GrpcClientBeanPostProcessorEx .
 */
public class GrpcClientBeanPostProcessorEx extends BeanPostShenyuClientRegister {
     ...
     */
    public GrpcClientBeanPostProcessorEx(final PropertiesConfig clientConfig, final ShenyuClientRegisterRepository shenyuClientRegisterRepository) {
    ...
    private List<MetaDataRegisterDTO> handler(final Object serviceBean) {
        ...
        return Arrays.stream(methods).filter(m -> m.isAnnotationPresent(ShenyuGrpcClient.class))
                .map(method -> {
                    ShenyuGrpcClient shenyuGrpcClient = method.getAnnotation(ShenyuGrpcClient.class);
    ...
    private MetaDataRegisterDTO buildMetaDataDTO(final String packageName, final ShenyuGrpcClient shenyuGrpcClient, final Method method) {
        String path = this.getContextPath() + shenyuGrpcClient.path();
